...
# same folder as this R project ("Bite_Force_Databank.Rproj").
...
# define lever.ratio if forceX system (s. Rühr & Blanke (2022) MEE)
lever.ratio <- 10.5 / 19.5 

# get names of corrected files
correct_names <- list.files("./2_corrected")
...
# re-rename amp column to amplification
iBite.table <- iBite.table %>% 
...
# Correct HemiHolo for Isoptera
iBite.table.reduced_iBite$HemiHolo[iBite.table.reduced_iBite$order == "Isoptera"] <- "Hemimetabola"


# reduce tibble to the one observation per specimen with the highest mean.bf.ID
...
# plot of climate coverage for talk
pie.plot.data <- iBite.table.climate.zone.bar %>% 
  group_by(kgcI) %>%
  mutate(sum_kgcI = sum(n.climate.zone)) %>% 
  distinct(kgcI, sum_kgcI) %>% 
  mutate(perc = round(sum_kgcI * 100/ sum(iBite.table.climate.zone.bar$n.climate.zone),1)) %>% 
  arrange(perc)

pie.plot.data$kgcI <- factor(c("tropical", "dry", "temperate", "continental"),
                             levels = c("tropical", "dry", "temperate", "continental"))

ggplot(pie.plot.data, aes(x = "", y = perc, fill = kgcI)) +
  geom_col(color = "black") +
  geom_label(aes(label = perc), color = c("white", "white", "white", "black"),
...
# # plot not used
# library(svglite)
# ggsave(filename = file.path("./4_plots",
#                             "coverage_climate_talk.svg"),
#        device = "svg", width = (21.0-4)/2, height = (29.7-4)/2, units = "cm")
